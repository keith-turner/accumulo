...
public class TabletRefresher {
  ...
  public static void refresh(ServerContext context, long fateTxid, TableId tableId, byte[] startRow,
    ...
    try (var tablets = context.getAmple().readTablets().forTable(tableId)
      ...
      Iterators.partition(tabletIterator, 1000).forEachRemaining(batch -> {
        ...
        try {
          refreshTablets(threadPool, fateTxid, context, refreshesNeeded);
        } catch (Exception e) {
  ...
  private static void refreshTablets(ThreadPoolExecutor threadPool, long tid, ServerContext context,
    ...
    while (!refreshesNeeded.isEmpty()) {
      ...
      for (Map.Entry<TabletMetadata.Location,List<TTabletRefresh>> entry : refreshesNeeded
        ...
        Future<List<TTabletRefresh>> future = threadPool
            .submit(() -> sendSyncRefreshRequest(context, tid, entry.getKey(), entry.getValue()));
      ...
      for (Map.Entry<TabletMetadata.Location,Future<List<TTabletRefresh>>> entry : futures
        ...
        List<TTabletRefresh> nonRefreshedExtents = future.get();
      ...
      if (!refreshesNeeded.isEmpty()) {
        LiveTServerSet lts = new LiveTServerSet(context, (current, deleted, added) -> {});
  ...
  private static List<TTabletRefresh> sendSyncRefreshRequest(ServerContext context, long tid,
