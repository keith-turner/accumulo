...
public class DeleteOperationIds extends ManagerRepo {
  ...
  public Repo<Manager> call(long tid, Manager manager) throws Exception {
    ...
    try (var tabletsMutator = manager.getContext().getAmple().conditionallyMutateTablets()) {
      ...
      tabletsMutator.mutateTablet(splitInfo.getLow()).requireOperation(opid).deleteOperation()
          .submit(unknownValidator);
      tabletsMutator.mutateTablet(splitInfo.getHigh()).requireOperation(opid).deleteOperation()
          .submit(unknownValidator);
      ...
      if (!allAccepted) {
        throw new IllegalStateException("Failed to delete operation ids " + splitInfo.getOriginal()
            + " " + splitInfo.getSplit() + " " + results.values().stream()
                .map(Ample.ConditionalResult::getStatus).collect(Collectors.toSet()));
      }
    }
