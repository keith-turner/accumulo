...
import org.apache.accumulo.server.validation.table.TableCryptoServiceValidator;
...
public interface TablePluginValidator {
  ...
  /**
   * Validate a plugin type based on current configuration for a given table. If something is not
   * valid, implementations should log any configuration issues at the warning level before finally
   * returning false.
   */
  ...
  static List<TablePluginValidator> validators() {
    return List.of(new CompactionDispatcherValidator(), new IteratorValidator(),
        ...
        new TableCryptoServiceValidator());
  }
}
